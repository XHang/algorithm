...
MD5算法用于对字符串生成摘要，可以用于区分不同的字符串，用于确保信息传输完整一致。

对于同一个字符串，生成的摘要始终是一样的。
...
1. 存储用户密码上
   ...
   由于MD5生成的摘要比较难反推回原始字符串（没有解密算法）

   所以一般用于将用户的密码用MD5生成摘要，并存储。

   这样除了用户，谁也不知道密码是多少了。

   用户登录时输入原始密码，然后用MD5生成摘要，最后和数据库匹配。

   就完成了密码的校验行为。

   > 由于不知道密码多少，所以忘记密码只能采取重置的行为

   但是这只能作为初级的密码加密方案。

   虽然MD5没有解密算法反推原始字符串。

   但是MD5生成的摘要数是有限的，而原始字符串的排列组合是无限的
...
2. 生成签名校验请求参数是否遭到篡改
   ...
   双方规定好将请求参数排列成指定的格式，再进行MD5算法取出摘要，作为签名去传递

   使攻击者就算能篡改请求参数，如果不知道签名的生成方式，也无法执行成功。

   > 执行程序会去校验签名和请求参数生成的签名是否一致。若不一致，即签名错误
   >
   > 会驳回

   当然，如果攻击者知道签名的生成方式，也就基本凉凉了~~~

   实际操作中，除了将请求参数排列成指定的格式外，还会加一些额外的信息

   （说白了也就是字符串，或者叫盐）

   作用是，即使攻击者知道签名算法怎么做的，不知道盐，也白给。
   ...
   目前看来，这个东西的功能就是用于存储多个种类的是/否标识位的
   ...
   后期标签会越来越多，然而你总不能在数据表增加N多的is_xxxx字段吧？
   
   这个时候，使用位掩码，用一个字段，里面存储一段数字.
